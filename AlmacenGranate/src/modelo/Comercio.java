...
public class Comercio extends Actor {
	...
	protected boolean  validarIdentificadorUnico(long identificador) throws Exception {
		...
		if(!lenghtCuit(miCuit)) throw new Exception("La longitud del cuit debe ser 11");
		
		if(!tipoXY(miCuit))throw new Exception("Los dos primeros numeros del cuit no son validos");
        ...
        } else throw new Exception ("El cuit ingresado es invalido");
		...
		while (index<this.lstDiaRetiro.size() && agregado==false) {
			if(this.lstDiaRetiro.get(index).getHoraDesde().equals(horaDesde) &&
					this.lstDiaRetiro.get(index).getHoraHasta().equals(horaHasta) &&
					  this.lstDiaRetiro.get(index).getDiaSemana()==diaSemana) throw new Exception("Error! Dia de la semana y hora no disponibles");
		index++;
		}
	...
	public String toString() {
		return "Comercio: Nombre del comercio=" + nombreComercio + ", cuit=" + cuit + ", costo Fijo=" + costoFijo
				...
				+ " Lista dia de retiro= " + lstDiaRetiro + " Lista Carrito=" + lstCarrito + ", id=" + id + ", contacto="
				+ contacto + ", getNombreComercio()=" + getNombreComercio() + ", getCuit()=" + getCuit()
				+ ", getCostoFijo()=" + getCostoFijo() + ", getCostoPorKm()=" + getCostoPorKm() + ", getDiaDescuento()="
				+ getDiaDescuento() + ", getPorcentajeDescuentoDia()=" + getPorcentajeDescuentoDia()
				+ ", getPorcentajeDescuentoEfectivo()=" + getPorcentajeDescuentoEfectivo() + "getLstDiaRetiro()="
				+ getLstDiaRetiro() + ", getLstCarrito()=" + getLstCarrito();
	}
   ...
   public boolean tipoXY(String cuit)  {
	   ...
	   return (tipoXY1.equalsIgnoreCase(TIPO_EMPRESA)|| 
			                  tipoXY1.equalsIgnoreCase(TIPO_FEMENINO)
